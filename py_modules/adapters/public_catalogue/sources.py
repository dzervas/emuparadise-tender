...
class PublicSourceStore:
    ...
    def find(self, catalogue: str, external_id: str) -> int | None:
        with closing(sqlite3.connect(self._db_path)) as conn:
            row = conn.execute(
                "SELECT rom_id FROM public_sources WHERE catalogue = ? AND external_id = ?", (catalogue, external_id)
            ).fetchone()
    ...
    def register(self, catalogue: str, external_id: str, detail: dict[str, Any], provider: str, page: str) -> int:
        with closing(sqlite3.connect(self._db_path, timeout=5)) as conn, conn:
            ...
            if existing:
                rom_id = existing[0]
                if conn.execute("SELECT 1 FROM rom_installs WHERE rom_id = ?", (rom_id,)).fetchone():
                    if (provider, page) != (existing[1], existing[2]):
                        raise ValueError("Delete the installed ROM before choosing a different download source")
                    return rom_id
                detail = {**detail, "id": rom_id}
                conn.execute(
                    "UPDATE public_sources SET detail_json = ?, download_provider = ?, download_page = ? "
                    "WHERE rom_id = ?",
                    (json.dumps(detail), provider, page, rom_id),
                )
                return rom_id
